...
int is_register_offset(uint32_t ins){//offset +- arm_read_register(p,Rm)
    if(get_bit(ins,25) == 1 && get_bits(ins,11,4)==0 ){
        return 1;
    }
    return 0;
}
...
//A5-20 p460
void determiner_addr(arm_core p,uint32_t ins,uint32_t *addr){
...
void determiner_mis_addr(arm_core p,uint32_t ins,uint32_t *addr){
...
int instruction_ldr(arm_core p, uint32_t ins){//A4-43 p193
    uint32_t word;
    uint32_t address = 0;
    determiner_addr(p, ins,&address);
    //MemoryAccess(B-bit, E-bit)
    
    arm_read_word(p, address, &word);//data = Memory[address,4]
    if(bits_rd(ins) == 15){
...
int instruction_ldrh(arm_core p, uint32_t ins){ 
    uint16_t half;
    uint32_t address = 0;
    determiner_mis_addr(p,ins,&address);
    //CP15_reg1_Ubit 
